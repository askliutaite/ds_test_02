...
author: "Aušrinė Skliutaitė, 1811366" 
...
```{r}
vidurkis <- aggregate(df$values, by=list(c(df$geo),(df$time)), FUN=mean, na.rm=TRUE)
...
```{r, fig.width=10}
ggplot(vidurkis, aes(Group.2, x, col=Group.1, group=Group.1))+
        geom_line()+
        geom_point()+
        scale_x_date(breaks="1 year",date_labels ="%Y")+
labs(title="Darbo pajamu ir BVP santykis", subtitle="Eurostat: namq_10_gdp", x="Metai", y="Vidurkis")
